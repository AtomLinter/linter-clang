    ...
    return {
      ...
      lint: (activeEditor) => {
        ...
        if(activeEditor.getGrammar().name === "C++") {
          //const language = "c++";
          args.push("-xc++");
          splited = atom.config.get("linter-clang.clangDefaultCppFlags").split(" ");
          args.push.apply(args, splited);
        }
        if(activeEditor.getGrammar().name === "Objective-C++") {
          //const language = "objective-c++";
          args.push("-xobjective-c++");
          splited = atom.config.get("linter-clang.clangDefaultObjCppFlags").split(" ");
          args.push.apply(args, splited);
        }
        if(activeEditor.getGrammar().name === "C") {
          //const language = "c";
          args.push("-xc");
          splited = atom.config.get("linter-clang.clangDefaultCFlags").split(" ");
          args.push.apply(args, splited);
        }
        if(activeEditor.getGrammar().name === "Objective-C") {
          //const language = "objective-c";
          args.push("-xobjective-c");
          splited = atom.config.get("linter-clang.clangDefaultObjCFlags").split(" ");
          args.push.apply(args, splited);
        }
